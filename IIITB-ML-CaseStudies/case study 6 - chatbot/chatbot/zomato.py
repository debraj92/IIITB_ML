...
class Zomato:
    ...
    def get_city_ID(self, city_name):
        ...
        if len(a['location_suggestions']) == 0:
            ...
            # always pick first result for now
    ...
    def restaurant_search(self, query="", city_id= "",
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&entity_id="
            + str(city_id) + "&entity_type=" + "city" + "&cuisines=" + str(cuisines) + "&sort="
    ...
    def is_rate_exceeded(self, a):
        ...
        if 'code' in a:
            if a['code'] == 440:
                print("Zomato api limit exceeded")
